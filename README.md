...
> **Note:** the `web` image is a multi-stage build — it compiles the Angular SPA and nginx serves it (see `deploy/nginx/Dockerfile`). The authentication, Teams, Epics, and Board screens (incl. ticket create/edit/delete, drag-and-drop, and state changes) are all implemented.
...
The Angular SPA lives in `frontend/` (Angular 22, Angular Material, standalone components, zoneless, signals). Implemented: the **user-authentication flow** ([Wireframe 2](docs/ui/wireframes/README.md)), **Teams management** ([Wireframe 4](docs/ui/wireframes/README.md)), **Epics management** ([Wireframe 5](docs/ui/wireframes/README.md)), and the **Kanban board** ([Wireframe 8](docs/ui/wireframes/README.md)) — team selector, five state columns, client-side Type/Epic/title filters, and full **ticket management** (create/edit/delete via a dialog, **drag-and-drop** between columns via `@angular/cdk/drag-drop`, plus a per-card state selector as a fallback) — all behind a shared navigation layout.
